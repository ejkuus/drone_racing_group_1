...
class LineObjective(object):
    ...
    def __init__(self):
        ...
        self.image_sub = rclpy.Subscriber("/camera/rgb/image_raw", Image, self.camera_callback)
    ...
    def camera_callback(self, data):
        ...
        if self.reached_target:
            rclpy.loginfo(f"Reached target color: {self.target_color}. Stopping.")
        ...
        if self.target_color in self.hsv_ranges:
            ...
            if cv2.countNonZero(target_mask) > 0:
                rclpy.loginfo(f"Detected target color: {self.target_color}.")
        ...
        rclpy.loginfo(f"Number of centroids: {len(contours)}")
        ...
        # rclpy.loginfo(f"ANGULAR VALUE SENT: {twist_object.angular.z}")
        # self.movekobuki_object.move_robot(twist_object)
def main():
    rclpy.init_node('line_objective_node', anonymous=True)

    line_objective_object = LineObjective()

    rate = rclpy.Rate(5)
    ctrl_c = False

    def shutdownhook():
        line_objective_object.clean_up()
        rclpy.loginfo("Shutdown time!")
        nonlocal ctrl_c
        ctrl_c = True

    rclpy.on_shutdown(shutdownhook)
